    ...
    float2 texCoord : TEXCOORD) : SV_TARGET
{
    ...
    // 如果亮度range低于一个与最大亮度呈正相关的阈值， 或者低于一个绝对阈值， 说明不是处于边缘区域， 不进行任何抗锯齿操作
    bool earlyExit = lumaRangeMax < max(g_QualityEdgeThresholdMin, lumaRangeMax * g_QualityEdgeThreshold);
...
/*--------------------------------------------------------------------------*/
    ...
    // 判断这是否为一个好的边界
    bool goodSpanN = (lumaEndN < 0.0) != isLumaMSmaller; // lumaN - lumaLocalAvg 和 lumaM - lumaLocalAvg是不是一个符号
