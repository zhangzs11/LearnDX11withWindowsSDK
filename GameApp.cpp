...
void GameApp::UpdateScene(float dt)
{
    ...
    {
        if (mousePos.x >= m_DebugTextureXY.x && mousePos.x < m_DebugTextureXY.x + m_DebugTextureWH.x && mousePos.y >= m_DebugTextureXY.y && mousePos.y < m_DebugTextureXY.y + m_DebugTextureWH.y)
            mouseStatus = 1;
        else
            mouseStatus = 0;
    }
